...
module.exports = {
    register: async (req, res) => {
        try {
            ...
            const user = await User.create({
                name, email, password: hashPassword, user_type, verify:false
            });
    ...
    verifikasiEmail: async (req, res) => {
        ...
        if (user) {
            ...
            const html = await nodemailer.getHtml('verifikasi.ejs', {name: user.name, url});
            nodemailer.sendMail(user.email, 'Verifikasi email', html);
        }
    ...
    verifyEmail: async (req, res) => {
        try {
            ...
            const updated = await User.update({verify: true}, {where: {id: data.id}});
            if (updated[0] == 0) {
                return res.render('auth/reset-password', {message: `reset password failed!`, token});
            }
    ...
    verifikasiPage: (req, res) => {
        const {token} = req.query;
        return res.render('emailtemplate/verifikasi_success', {message: null, token});
    },
};